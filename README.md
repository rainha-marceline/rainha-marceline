<div align="center">

# Oi! Eu sou a Marceline 🌸

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=20&pause=1000&color=FF69B4&center=true&vCenter=true&width=600&lines=Aprendendo+Python+%F0%9F%90%8D;Sonhando+em+ser+Dev+Backend+%E2%9C%A8;Cada+linha+de+c%C3%B3digo+%C3%A9+um+passo+%F0%9F%8C%B8)](https://git.io/typing-svg)

</div>

---

## 🌷 Sobre mim

Olá! Me chamo **Marceline** e estou dando meus primeiros passos no mundo da programação. 🐣

Atualmente estou aprendendo **Python** e tenho um grande sonho: me tornar uma **desenvolvedora backend**! 💜

```python
marceline = {
    "nome":        "Marceline",
    "aprendendo":  "Python 🐍",
    "sonho":       "Ser Dev Backend ✨",
    "status":      "iniciante com muito gás 🔥",
    "humor":       "Bugs? A gente resolve! 💪",
}
```

---

## 💻 Aprendendo agora

![Python](https://img.shields.io/badge/Python-FFB6C1?style=for-the-badge&logo=python&logoColor=white)

---

## 🌸 No meu horizonte

Tecnologias que quero aprender no futuro:

![FastAPI](https://img.shields.io/badge/FastAPI-FFB6C1?style=for-the-badge&logo=fastapi&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-FFB6C1?style=for-the-badge&logo=postgresql&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-FFB6C1?style=for-the-badge&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-FFB6C1?style=for-the-badge&logo=git&logoColor=white)

---

## 💌 Me encontra aqui

[![GitHub](https://img.shields.io/badge/GitHub-rainha--marceline-FFB6C1?style=for-the-badge&logo=github&logoColor=white)](https://github.com/rainha-marceline)

---

<div align="center">

*"Toda especialista já foi um dia iniciante." 🌱*

✨ **construindo meu caminho, um `print()` de cada vez** ✨

</div>
